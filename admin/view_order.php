...
<section class="order-details">
   ...
   <?php
      ...
      if($select_orders->rowCount() > 0){
         while($fetch_order = $select_orders->fetch(PDO::FETCH_ASSOC)){
            $select_product = $conn->prepare("SELECT * FROM `tbl_product` WHERE product_id = ? LIMIT 1");
   ...
   <div class="box">
      <div class="col">
         ...
         <p class="price"><i class="fa-solid fa-coins"></i> <?= $fetch_order['price']; ?> x <?= $fetch_order['qty']; ?></p>
         <h3 class="name"><?= $fetch_product['product_name']; ?></h3>
         <p class="grand-total">grand total : <span><i class="fa-solid fa-coins"></i><?= $grand_total; ?></span></p>
      </div>
      <div class="col">
