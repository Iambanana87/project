...
if(isset($_POST['place_order'])){
   ...
   if(isset($_GET['get_id'])){
      ...
      $get_product = $conn->prepare("SELECT * FROM `tbl_product` WHERE product_id = ? LIMIT 1");
      ...
      if($get_product->rowCount() > 0){
         while($fetch_p = $get_product->fetch(PDO::FETCH_ASSOC)){
            ...
            $insert_order->execute([create_unique_id(), $user_id, $name, $number, $email, $address, $address_type, $method, $fetch_p['product_id'], $fetch_p['product_price'], 1]);
...
if(isset($_POST['delete_item'])){
   ...
   $verify_delete_item = $conn->prepare("SELECT * FROM `order` WHERE id = ?");
   $verify_delete_item->execute([$cart_id]);

   if($verify_delete_item->rowCount() > 0){
      $delete_cart_id = $conn->prepare("DELETE FROM `order` WHERE id = ?");
      $delete_cart_id->execute([$cart_id]);
      $success_msg[] = 'Order item deleted!';
   }else{
      $warning_msg[] = 'Order item already deleted!';
   } 
   ...
   <div class="row">
      ...
      <form action="" method="POST">
         ...
         <div class="flex">
            <div class="box">
               ...
               <p>Nhập số điện thoại <span>*</span></p>
               ...
               <select name="method" class="input" required>
                  <option value="cash on delivery">Thanh toán MoMo</option>
                  <option value="credit or debit card">Thanh toán bằng thẻ trả sau</option>
                  <option value="net banking">Thanh toán bằng thẻ trả sau</option>
                  <option value="UPI or wallets">Thanh toán bằng thẻ nội địa</option>
               </select>
               </div>
               <div class="box">
               ...
               <input type="text" name="flat" required maxlength="50" placeholder="tên đường & số nhà " class="input">
      ...
      <div class="summary">
         ...
         <?php
            $grand_total = 0;
            if(isset($_GET['get_id'])){
               $select_get = $conn->prepare("SELECT * FROM `tbl_product` WHERE product_id = ?");
         ...
         <img src="uploads/<?= $fetch_product['product_img']; ?>" class="image" alt="<?= $fetch_product['product_name']; ?>">
            <div>
               <h3 class="name"><?= $fetch_get['product_name']; ?></h3>
               <p class="price"><i class="fa-solid fa-coins"></i> <?= $fetch_get['product_price']; ?> x 1</p>
            </div>
         </div>
         <?php
               }
               ...
               if($select_cart->rowCount() > 0){
                  while($fetch_cart = $select_cart->fetch(PDO::FETCH_ASSOC)){
                     $select_products = $conn->prepare("SELECT * FROM `tbl_product` WHERE product_id = ?");
         ...
         <img src="uploads/<?= $fetch_product['product_img']; ?>" class="image" alt="<?= $fetch_product['product_name']; ?>">
            <div>
               ...
               <p class="price"><i class="fa-solid fa-coins"></i> <?= $fetch_product['product_price']; ?> x <?= $fetch_cart['qty']; ?></p>
            </div>
         </div>
         <?php
                  }
         ...
         ?>
         <div class="grand-total"><span>grand total :</span><p><i class="fa-solid fa-coins"></i><?= $grand_total; ?></p></div>
      </div>
